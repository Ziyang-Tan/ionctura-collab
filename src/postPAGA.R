...
data_dir = './PAGA_result_data_manual_gating_asin_scale/'

#for (tumor_group in c('Tumor Group 3')){
tumor_group = 'Tumor Group 3'  
sub_names <- unique(sapply(dir(path=paste0(data_dir, '/', tumor_group)), 
                           function(x){strsplit(x,split = '_')[[1]][3]}))
for (sub_name in sub_names){
  result_dir = file.path('./figures_manual_gating_asin_scale_2', tumor_group , sub_name)
  ...
  data_freq = meta %>% 
    ...
    mutate(freq = n / sum(n)) %>%
    filter(timepoint %in% c('AC1D1', 'AC2D1', 'AC3D1', 'AC4D1', 'AC5D1', 'AC6D1', 'AC7D1'))
  ...
  # boxplot
  g_list <- lapply(sort(unique(meta$leiden)), function(x){
    ggplot(data_freq %>% filter(leiden == x), 
           aes(x=timepoint, y=freq)) +
  ...
  ggarrange(plotlist = g_list, ncol = 4, nrow = 3) %>% 
    ggexport(filename = file.path(result_dir, paste0(sub_name, '_cluster_changes_timepoint_boxplot.pdf')),
             width = 20,
             height = 10)
  ################################
